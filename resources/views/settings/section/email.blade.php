    ...
    <div class="flex flex-col lg:flex-row gap-8 items-start p-6 bg-gray-50 rounded-2xl shadow-sm">
        ...
        <div class="w-full lg:w-1/4 sticky top-4">
            ...
            <div class="mt-4 p-4 bg-white rounded-xl shadow-sm border border-gray-200">
                <h3 class="text-sm font-semibold text-gray-800 flex items-center">
                    <i class="mdi mdi-lightbulb-outline mr-2 text-indigo-600"></i>
                    {{ __('Quick Tips') }}
                </h3>
        ...
        <div class="w-full lg:w-3/4 space-y-6">
            ...
            <div
                ...
                <div class="flex items-center mb-4">
                    <div class="flex items-center justify-center w-10 h-10 bg-indigo-100 rounded-lg mr-3">
                        <i class="mdi mdi-email-send text-indigo-600 text-xl"></i>
                    </div>
                    <h3 class="text-lg font-semibold text-gray-800">{{ __('Mailer Configuration') }}</h3>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {{-- Default Mailer --}}
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-gray-800 mb-2">{{ __('Default Mailer') }}</label>
                        <select name="mail[default]" id="mail_selector"
                            class="w-full px-4 py-3 rounded-lg border border-gray-300 shadow-sm focus:border-indigo-600 focus:ring-2 focus:ring-indigo-100 transition-colors duration-300">
                    ...
                    {{-- From Address --}}
                    <div>
                        <label class="block text-sm font-medium text-gray-800 mb-2">{{ __('From Email') }}</label>
                        <div class="relative">
                            <input type="email" name="mail[from][address]" value="{{ config('mail.from.address') }}"
                                class="w-full pl-10 pr-4 py-3 rounded-lg border border-gray-300 shadow-sm focus:border-indigo-600 focus:ring-2 focus:ring-indigo-100 transition-colors duration-300"
                    ...
                    {{-- From Name --}}
                    <div>
                        <label class="block text-sm font-medium text-gray-800 mb-2">{{ __('From Name') }}</label>
                        <div class="relative">
                ...
                <div class="mt-6 pt-4 border-t border-gray-200">
                    {{-- SMTP --}}
                    <div id="smtp"
                        class="mail-config p-5 bg-gray-50 rounded-xl shadow-sm border border-gray-200 hidden">
                        <h4 class="text-md font-semibold text-gray-800 mb-4 flex items-center">
                            <i class="mdi mdi-server mr-2 text-indigo-600"></i>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            @foreach (['host', 'port', 'encryption', 'username', 'password', 'timeout', 'local_domain'] as $key)
                                <div class="{{ in_array($key, ['timeout', 'local_domain']) ? 'md:col-span-2' : '' }}">
                                    <label
                                        class="block text-sm font-medium text-gray-800 mb-2">{{ ucfirst(str_replace('_', ' ', $key)) }}</label>
                                    <div class="relative">
                                        ...
                                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                            <i
                                                class="mdi mdi-{{ $key === 'password' ? 'key-variant' : ($key === 'username' ? 'account' : ($key === 'host' ? 'server' : ($key === 'port' ? 'numeric' : 'lock'))) }} text-gray-400"></i>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
                    ...
                    <div id="mailgun"
                        class="mail-config p-5 bg-gray-50 rounded-xl shadow-sm border border-gray-200 hidden">
                        <h4 class="text-md font-semibold text-gray-800 mb-4 flex items-center">
                            <i class="mdi mdi-email-fast-outline mr-2 text-indigo-600"></i>
                            {{ __('Mailgun Configuration') }}
                        </h4>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            @foreach (['domain', 'secret', 'endpoint', 'scheme'] as $key)
                                <div>
                                    <label class="block text-sm font-medium text-gray-800 mb-2">{{ ucfirst($key) }}</label>
                                    <div class="relative">
                                        ...
                                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                            <i
                                                class="mdi mdi-{{ $key === 'secret' ? 'key-variant' : ($key === 'domain' ? 'web' : 'settings') }} text-gray-400"></i>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
                    ...
                    <div id="ses"
                        class="mail-config p-5 bg-gray-50 rounded-xl shadow-sm border border-gray-200 hidden">
                        <h4 class="text-md font-semibold text-gray-800 mb-4 flex items-center">
                            <i class="mdi mdi-amazon mr-2 text-indigo-600"></i>
                            ...
                            @foreach (['key', 'secret', 'region'] as $key)
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                            <i
                                                class="mdi mdi-{{ $key === 'secret' ? 'key-variant' : ($key === 'key' ? 'key' : 'earth') }} text-gray-400"></i>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
                    ...
                    <div id="postmark"
                        class="mail-config p-5 bg-gray-50 rounded-xl shadow-sm border border-gray-200 hidden">
                        <h4 class="text-md font-semibold text-gray-800 mb-4 flex items-center">
                            <i class="mdi mdi-email-check-outline mr-2 text-indigo-600"></i>
                            {{ __('Postmark Configuration') }}
                        </h4>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="md:col-span-2">
                                <label class="block text-sm font-medium text-gray-800 mb-2">{{ __('Token') }}</label>
                                <div class="relative">
            ...
            <div
                ...
                <div class="flex items-center mb-4">
                    <div class="flex items-center justify-center w-10 h-10 bg-indigo-100 rounded-lg mr-3">
                        <i class="mdi mdi-timer-sand text-indigo-600 text-xl"></i>
                    </div>
                    <h3 class="text-lg font-semibold text-gray-800">{{ __('Expiration Settings') }}</h3>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label
                            class="block text-sm font-medium text-gray-800 mb-2">{{ __('Account Verification Time') }}</label>
                        <div class="relative">
                    ...
                    <div>
                        <label
                            class="block text-sm font-medium text-gray-800 mb-2">{{ __('2FA Email Code Expiration') }}</label>
                        <div class="relative">
                    ...
                    <div>
                        <label
                            class="block text-sm font-medium text-gray-800 mb-2">{{ __('Password Reset Expiration') }}</label>
                        <div class="relative">
                    ...
                    <div>
                        <label
                            class="block text-sm font-medium text-gray-800 mb-2">{{ __('Password Reset Throttle') }}</label>
                        <div class="relative">
