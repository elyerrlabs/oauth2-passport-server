            ...
            <div
                ...
                <div class="flex items-center mb-4">
                    <div
                        ...
                        <i class="mdi mdi-playlist-star text-indigo-600 dark:text-indigo-400 text-xl"></i>
                    </div>
                ...
                <div class="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <div id="database_queue"
                        class="queue-config p-5 bg-gray-50 dark:bg-gray-700 rounded-xl shadow-sm border border-gray-200 dark:border-gray-600 hidden transition-colors duration-300">
                                ...
                                <div class="{{ in_array($key, ['retry_after']) ? 'md:col-span-2' : '' }}">
                                    ...
                                    <div class="relative">
                                        <input type="{{ $key === 'retry_after' ? 'number' : 'text' }}"
                                            ...
                                            class="w-full pl-10 pr-4 py-3 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-600 text-gray-900 dark:text-gray-100 shadow-sm focus:border-indigo-500 dark:focus:border-indigo-400 focus:ring-2 focus:ring-indigo-200 dark:focus:ring-indigo-800 transition-colors duration-300"
                                        ...
                                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                            <i
                                                class="mdi mdi-{{ $key === 'table' ? 'table' : ($key === 'queue' ? 'playlist-play' : ($key === 'retry_after' ? 'timer-outline' : 'check-circle')) }} text-gray-500 dark:text-gray-400"></i>
                                        </div>
                                    </div>
                                    @if ($key === 'retry_after')
                            ...
                            @foreach (['host', 'queue', 'retry_after', 'block_for', 'after_commit'] as $key)
                                <div class="{{ in_array($key, ['retry_after', 'block_for']) ? 'md:col-span-2' : '' }}">
                                    ...
                                    <div class="relative">
                                        ...
                                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                            <i
                                                class="mdi mdi-{{ $key === 'host' ? 'server' : ($key === 'queue' ? 'playlist-play' : ($key === 'retry_after' ? 'timer-outline' : ($key === 'block_for' ? 'clock-outline' : 'check-circle'))) }} text-gray-500 dark:text-gray-400"></i>
                                        </div>
                                    </div>
                                    @if ($key === 'retry_after')
                                        <small class="block mt-1 text-sm text-gray-500 dark:text-gray-400">
                            ...
                            @foreach (['key', 'secret', 'prefix', 'queue', 'suffix', 'region', 'after_commit'] as $key)
                                <div class="{{ in_array($key, ['key', 'secret']) ? 'md:col-span-2' : '' }}">
                                    ...
                                    <div class="relative">
                                        ...
                                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                            <i
                                                class="mdi mdi-{{ $key === 'key' ? 'key' : ($key === 'secret' ? 'key-variant' : ($key === 'region' ? 'earth' : ($key === 'queue' ? 'playlist-play' : 'format-letter-case'))) }} text-gray-500 dark:text-gray-400"></i>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
                            ...
                            @foreach (['connection', 'queue', 'retry_after', 'block_for', 'after_commit'] as $key)
                                <div class="{{ in_array($key, ['retry_after', 'block_for']) ? 'md:col-span-2' : '' }}">
                                    ...
                                    <div class="relative">
                                        ...
                                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                            <i
                                                class="mdi mdi-{{ $key === 'connection' ? 'connection' : ($key === 'queue' ? 'playlist-play' : ($key === 'retry_after' ? 'timer-outline' : ($key === 'block_for' ? 'clock-outline' : 'check-circle'))) }} text-gray-500 dark:text-gray-400"></i>
                                        </div>
                                    </div>
                                    @if ($key === 'retry_after')
                                        <small class="block mt-1 text-sm text-gray-500 dark:text-gray-400">
            ...
            <div
                ...
                <div class="flex items-center mb-4">
                    <div class="flex items-center justify-center w-10 h-10 bg-blue-100 dark:bg-blue-900 rounded-lg mr-3">
                        <i class="mdi mdi-worker text-blue-500 dark:text-blue-400 text-xl"></i>
                    </div>
                    <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-200">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <div class="relative">
                            <input type="number" name="queue[timeout]"
                                class="w-full pl-12 pr-4 py-3 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 shadow-sm focus:border-indigo-500 dark:focus:border-indigo-400 focus:ring-2 focus:ring-indigo-200 dark:focus:ring-indigo-800 transition-colors duration-300"
                                value="{{ config('queue.timeout', 60) }}" min="1" placeholder="60">
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                <span class="text-gray-500 dark:text-gray-400 text-sm">sec</span>
                            </div>
                        </div>
                        <small class="block mt-1 text-sm text-gray-500 dark:text-gray-400">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                <i class="mdi mdi-repeat text-gray-500 dark:text-gray-400"></i>
                            </div>
                        </div>
                        <small class="block mt-1 text-sm text-gray-500 dark:text-gray-400">
