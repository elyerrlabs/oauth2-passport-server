...
<template>
    <v-main-layout>
        <v-head
            :title="
                __('Manage service permissions for: :name', {
                    ':name': service.name,
                })
            "
            :description="__('Group :group', { ':group': service.group?.name })"
        >
            <template #actions>
                <v-button
                    as="a"
                    :to="page.props.routes.services"
                    left-icon="mdi mdi-arrow-left"
                    :label="__('Back to the services')"
                    variant="secondary"
                />

                <v-add-scope
                    :link="page.props.routes.scopes"
                    @created="getScopes"
                />
            </template>
        </v-head>
        ...
        <div class="p-6 overflow-y-auto">
            ...
            >
                ...
                >
                    ...
                    >
                        <v-delete-scope :scope="item" @deleted="getScopes" />
                        <v-add-scope
